...
public class Arquivo<T extends Registro, I extends RegistroHashExtensivel<I>> {
    ...
    public void createNewDir() throws Exception {
        ...
        // Pasta onde o arquivo .db deve ficar
        File pastaArquivo = new File(new File(this.nomeArquivo).getParent());
        if (!pastaArquivo.exists())
            pastaArquivo.mkdir();

        // Apaga arquivos de índice antigos (reset total)
        new File(pastaArquivo, "diretorio.db").delete();
        new File(pastaArquivo, "cestos.db").delete();
        ...
        this.indice = new HashExtensivel<>(
                construtorIndice,
                3,
                pastaArquivo.getPath() + "/diretorio.db",
                pastaArquivo.getPath() + "/cestos.db");
