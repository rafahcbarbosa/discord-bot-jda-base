...
public class LZW {
    ...
    public static ArrayList<byte[]> loadLZWEntries(File file) throws Exception {
        ArrayList<byte[]> list = new ArrayList<>();

        try (DataInputStream in = new DataInputStream(new FileInputStream(file))) {
            int fileBits = in.readInt();
            if (fileBits != BITS_POR_INDICE)
    ...
    public static void saveLZWEntries(ArrayList<byte[]> entries, File file) throws Exception {
        try (DataOutputStream out = new DataOutputStream(new FileOutputStream(file))) {
            out.writeInt(BITS_POR_INDICE);
            out.writeInt(entries.size());

            for (byte[] encodedBytes : entries) {
